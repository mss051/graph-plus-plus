...
exports.SMART_GRAPH_VIEW_TYPE = void 0;
const obsidian_1 = require("obsidian");
// ---------------------------------------------------------
// 1. DATA STRUCTURES & SETTINGS
// ---------------------------------------------------------
exports.SMART_GRAPH_VIEW_TYPE = "smart-graph-view";
...
// 2. MAIN PLUGIN CLASS
// ---------------------------------------------------------
class SmartGraphPlugin extends obsidian_1.Plugin {
    settings;
    async onload() {
        await this.loadSettings();
        this.registerView(exports.SMART_GRAPH_VIEW_TYPE, (leaf) => new SmartGraphView(leaf, this));
    ...
    async saveSettings(refreshGraphs = true) {
        ...
        if (refreshGraphs) {
            const leaves = this.app.workspace.getLeavesOfType(exports.SMART_GRAPH_VIEW_TYPE);
            if (leaves.length > 0) {
    ...
    async activateView() {
        ...
        let leaf = null;
        const leaves = workspace.getLeavesOfType(exports.SMART_GRAPH_VIEW_TYPE);
        if (leaves.length > 0) {
            leaf = leaves[0];
        }
        else {
            ...
            if (leaf) {
                await leaf.setViewState({ type: exports.SMART_GRAPH_VIEW_TYPE, active: true });
            }
        }
        if (leaf) {
    ...
    updateAllLocalGraphs() {
        ...
        markdownLeaves.forEach(leaf => {
            ...
            let wrapper = contentEl.querySelector('.smart-local-graph-wrapper');
    ...
    async runClusterAnalysis() {
        new obsidian_1.Notice("Starting network analysis...");
        const graph = this.buildBidirectionalGraph();
        if (graph.size === 0) {
            new obsidian_1.Notice("The graph is empty or has no links.");
            return;
        }
        ...
        if (hubPaths.length === 0) {
            new obsidian_1.Notice("Non ci sono abbastanza collegamenti nel Vault per creare dei gruppi.");
            return;
        }
        ...
        new obsidian_1.Notice(`Analysis complete! Detected ${hubPaths.length} macro-groups and ${finalCenters.length} absolute centers.`);
    }
    ...
    identifyHubsViaCommunities(graph) {
        ...
        for (const [label, members] of communities.entries()) {
            if (members.length < 4 && nodes.length > 20)
                continue;
    ...
    updateClusters(newHubPaths, graph) {
...
exports.default = SmartGraphPlugin;
// ---------------------------------------------------------
// 5. SETTINGS PANEL 
// ---------------------------------------------------------
class SmartGraphSettingTab extends obsidian_1.PluginSettingTab {
    plugin;
...
class SmartGraphView extends obsidian_1.ItemView {
    plugin;
    canvas;
    ctx;
    animationFrameId = 0;
    ...
    getViewType() {
        return exports.SMART_GRAPH_VIEW_TYPE;
    }
    getDisplayText() {
        return "Smart Graph";
    }
    ...
    async onOpen() {
        ...
        this.canvas.addEventListener('mousedown', (e) => {
            ...
            for (let i = this.nodes.length - 1; i >= 0; i--) {
                ...
                if (this.plugin.settings.spawnAnimation === 'hierarchical' && !node.isActive)
                    continue;
    ...
    updateNodeRadii() {
        for (const node of this.nodes) {
            ...
            if (node.isHub)
                calculatedRadius = Math.max(calculatedRadius, this.plugin.settings.nodeMaxRadius * 0.7);
            let radius = Math.min(calculatedRadius, this.plugin.settings.nodeMaxRadius);
            if (node.isAbsoluteCenter)
                radius = this.plugin.settings.nodeMaxRadius * 1.2;
            node.radius = radius;
        }
        this.wakeUp();
    }
    resolveCSSColor(cssVar, fallback) {
    ...
    getTextCanvas(text, fontStyle, fontSize, color, scaleLevel) {
    ...
    buildOverlayUI(container) {
        ...
        (0, obsidian_1.setIcon)(toggleBtn, 'settings');
        ...
        (0, obsidian_1.setIcon)(recalcBtnHeader, 'wand');
        ...
        (0, obsidian_1.setIcon)(resetBtn, 'rotate-ccw');
        ...
        (0, obsidian_1.setIcon)(closeBtn, 'x');
        ...
        const createSection = (titleText, defaultOpen = true) => {
        ...
        const createSelect = (parent, name, options, value, onChange) => {
        ...
        const createSlider = (parent, name, min, max, step, value, onChange) => {
            const wrapper = parent.createEl('div');
            wrapper.style.display = 'flex';
        ...
        const createToggle = (parent, name, value, onChange) => {
            const wrapper = parent.createEl('div');
            wrapper.style.display = 'flex';
            wrapper.style.justifyContent = 'space-between';
            wrapper.style.alignItems = 'center';
        ...
        const createTextInput = (parent, name, placeholder, value, onChange) => {
            const wrapper = parent.createEl('div');
            wrapper.style.display = 'flex';
            wrapper.style.flexDirection = 'column';
            wrapper.style.gap = '6px';
        ...
        const renderControls = () => {
            ...
            createSelect(displaySection, 'Spawn Animation', { radial: 'Radial Burst', hierarchical: 'Hierarchical Wave' }, this.plugin.settings.spawnAnimation, async (val) => {
                this.plugin.settings.spawnAnimation = val;
    ...
    stepPhysics(isPrecalc = false) {
        ...
        for (let i = 0; i < this.nodes.length; i++) {
            ...
            if (!n1.isActive && !isPrecalc)
                continue;
            for (let j = i + 1; j < this.nodes.length; j++) {
                const n2 = this.nodes[j];
                if (!n2.isActive && !isPrecalc)
                    continue;
                const dx = n1.x - n2.x;
                const dy = n1.y - n2.y;
                let distSq = dx * dx + dy * dy;
                if (distSq < GRAPH_CONSTANTS.PHYSICS.MIN_DIST_SQ)
                    distSq = GRAPH_CONSTANTS.PHYSICS.MIN_DIST_SQ;
        ...
        for (const edge of this.edges) {
            ...
            if ((!n1.isActive || !n2.isActive) && !isPrecalc)
                continue;
            const dx = n2.x - n1.x;
            const dy = n2.y - n1.y;
            ...
            if (force > GRAPH_CONSTANTS.PHYSICS.MAX_FORCE)
                force = GRAPH_CONSTANTS.PHYSICS.MAX_FORCE;
            if (force < -GRAPH_CONSTANTS.PHYSICS.MAX_FORCE)
                force = -GRAPH_CONSTANTS.PHYSICS.MAX_FORCE;
        ...
        for (const node of this.nodes) {
            ...
            if (Math.abs(node.vx) < GRAPH_CONSTANTS.PHYSICS.SLEEP_VELOCITY_THRESHOLD)
                node.vx = 0;
            if (Math.abs(node.vy) < GRAPH_CONSTANTS.PHYSICS.SLEEP_VELOCITY_THRESHOLD)
                node.vy = 0;
            node.x += node.vx;
            node.y += node.vy;
    ...
    initPhysicsData() {
        ...
        tempNodes.sort((a, b) => {
            if (a.isAbsoluteCenter && !b.isAbsoluteCenter)
                return -1;
            if (!a.isAbsoluteCenter && b.isAbsoluteCenter)
                return 1;
            if (a.isHub && !b.isHub)
    ...
    updatePhysics() {
        ...
        if (!this.isFullySpawned && this.plugin.settings.spawnAnimation === 'radial') {
            for (const node of this.nodes) {
                if (!node.isActive || node === this.draggedNode)
                    continue;
    ...
    drawGraph() {
        ...
        for (const node of this.nodes) {
            ...
            if (isHovered)
                targetRadius = node.radius * GRAPH_CONSTANTS.VISUALS.NODE_HOVER_SCALE;
            else if (isGroupHighlight)
                targetRadius = node.radius * GRAPH_CONSTANTS.VISUALS.NODE_GROUP_HIGHLIGHT_SCALE;
            ...
            if (this.plugin.settings.enableGlow) {
                if (node.isAbsoluteCenter)
                    targetGlow = GRAPH_CONSTANTS.VISUALS.CENTER_GLOW_BLUR * this.plugin.settings.glowIntensity;
                else if (isGroupHighlight && node.primaryColor)
                    targetGlow = GRAPH_CONSTANTS.VISUALS.GROUP_HIGHLIGHT_GLOW_BLUR * this.plugin.settings.glowIntensity;
            }
        ...
        const renderNode = (node, isHovered, isGroupHighlight) => {
...
class SmartLocalGraphRenderer {
    ...
    stableFrames = 0;
    energy = 1.0;
    constructor(container, file, plugin) {
    ...
    initData() {
        ...
        for (const path of localNodesSet) {
            ...
            if (isCenter)
                radius = this.plugin.settings.nodeMaxRadius * 1.0;
            this.nodes.push({
                id: path,
                name,
                color,
        ...
        this.canvas.addEventListener('mouseup', (e) => {
            ...
            if (dist < 5 && (this.draggedNode || this.hoveredNode)) {
                ...
                if (!targetNode.isCenter) {
                    ...
                    if (fileToOpen instanceof obsidian_1.TFile) {
